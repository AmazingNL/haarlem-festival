    ...
    <div class="shop-container">
        <article class="shop-card shop-card--success">
            <div class="shop-card__body">
                ...
                <?php if (($emailSentTo ?? '') !== ''): ?>
                    <p class="shop-status-note shop-status-note--success">
                        Confirmation email with invoice and tickets sent to <?= htmlspecialchars($emailSentTo, ENT_QUOTES, 'UTF-8') ?>.
                    </p>
                    <?php if (!empty($showMailpitLink)): ?>
                        <p class="shop-status-note">
                            <a href="http://localhost:8026" target="_blank" rel="noopener">Open Mailpit inbox</a>
                        </p>
                    <?php endif; ?>
                <?php elseif (($emailError ?? '') !== ''): ?>
                    <p class="shop-status-note shop-status-note--error">
                        We could not send the confirmation email. Your order is saved — use the invoice and QR codes below.
                    </p>
                <?php endif; ?>
                ...
                <div class="shop-total-card__actions">
                    <a href="/orders/<?= $orderId ?>/invoice" class="shop-button" target="_blank" rel="noopener">Open invoice (PDF)</a>
